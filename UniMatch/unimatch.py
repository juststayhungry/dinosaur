...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x,_),
            ...
            if max(pred_u_w_counter.values()) < total_u_pixel_num:  #
                if args.thresh_warmup:
                    for i in range(args.num_classes):
                        class_acc[i] = pred_u_w_counter[i] / max(pred_u_w_counter.values())#最大值归一化后各类别(num_classes中的类别)难度系数
                else:
            ...
            mask = conf_u_w.ge(T).float()  # convex
            ...
            mask_u_w矩阵是类别id矩阵
            mask_u_w.long()[select == 1]选出达到阈值的类别，矩阵

            '''
            mask_u_w_f = mask_u_w.long()[select == 1].flatten(1) 
            # select = select.flatten()
            for i,j in enumerate(x_ulb_idx):#每图片单独统计
                selected_label[x_ulb_idx] = mask_u_w_f[i]
